...
def main() -> None:
    parser = argparse.ArgumentParser(description="Corrige orientacao e preenchimento da tela no AgroFarm mobile")
    ...
    text, count = re.subn(r'html/head_include=".*?"', replacement, text, count=1)
    if count != 1:
    ...
    print(f"Orientacao paisagem e tela cheia aplicadas em: {root}")
