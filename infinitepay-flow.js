(() => {
  const STORAGE_KEY = 'cancao_order_v4';
  ...
  function renderVerifying() {
    ...
    card.innerHTML = `
      ...
      <div class="payment-confirmed-card" style="background:#fff8e8;border-color:rgba(140,100,30,.18)">
        <strong>Estamos consultando a InfinitePay...</strong>
        <p>Não feche esta página enquanto confirmamos o recebimento de ${PRICE_LABEL}.</p>
      </div>
  ...
  function renderPaid(order, payment) {
    ...
    card.innerHTML = `
      ...
      <div class="payment-confirmed-card">
        ...
        <strong>Pedido liberado para criação da letra.</strong>
        <p>Primeiro criaremos a letra. Você poderá solicitar até 3 edições antes de autorizar a geração da música.</p>
      </div>
      ...
      <button class="button button-primary form-submit" type="button" id="ip-send-order">Enviar dados do pedido</button>
      <p class="form-help">O pagamento já foi validado diretamente com a InfinitePay.</p>
    `;
  ...
  async function handleInfinitePayReturn() {
    ...
    try {
      if (!order) throw new Error('Não encontramos os dados do pedido neste dispositivo.');
      ...
      if (!response.ok || !result.success || !result.verified) {
        throw new Error(result.message || 'O PIX ainda não foi confirmado pela InfinitePay.');
      }

      const verifiedPayment = { ...payment, ...result };
